...
print(' ')
print('步骤：1.在原始台账表格所有页签之后新增页签，命名规则为Sheet1,Sheet2...')
print('步骤：2.在每个页签内：A列第一行起始日、A列第二行终止日、A列第三行台账总公司数')
print(' ')

#input name
excelname = input('表格名称(以".xls"格式):')

#open workbook and sheets
oriworkbook = xlrd.open_workbook(excelname)
workbook = copy(oriworkbook)
list_of_all_sheetnames = oriworkbook.sheet_names()

#get the dates
date_sheet = oriworkbook.sheet_by_name(list_of_all_sheetnames[len(list_of_all_sheetnames)-1])

startdate_original = xlrd.xldate_as_tuple(date_sheet.cell(0,0).value,0)
startdate = startdate_original[0]*10000 + startdate_original[1]*100 + startdate_original[2]*1

enddate_original = xlrd.xldate_as_tuple(date_sheet.cell(1,0).value,0)
enddate = enddate_original[0]*10000 + enddate_original[1]*100 + enddate_original[2]*1

total_sheet_num = int(date_sheet.cell(2,0).value)
...
for i in range(0, total_sheet_num):
    sheet = oriworkbook.sheet_by_name(list_of_all_sheetnames[i])
...
#change the excel
ws = workbook.get_sheet(len(list_of_all_sheetnames)-1)
ws.write(0, 0, label=startdate)
ws.write(1, 0, label=enddate)

#write a new excel sheet
ws.write(3, 0, label='总计')
ws.write(4, 0, label='公司')
ws.write(4, 1, label='租金总额')
ws.write(4, 2, label='本金总额')
ws.write(4, 3, label='收益总额')

for m in range(0,total_sheet_num):
    ws.write(m+1+3+1, 0, label=list_of_all_sheetnames[m])

for n in range(0,len(list_of_sum_in_dic)):
    ws.write(n+1+3+1, 1, label=list_of_sum_in_dic[n])

for f in range(0, len(list_of_sum_in_dic_princ)):
    ws.write(f+1+3+1, 2, label=list_of_sum_in_dic_princ[f])

for g in range(0, len(list_of_sum_in_dic_interest)):
    ws.write(g+1+3+1, 3, label=list_of_sum_in_dic_interest[g])

ws.write(total_sheet_num+1+3+1, 0, label='总计')
ws.write(total_sheet_num+1+3+1, 1, label=total)
ws.write(total_sheet_num+1+3+1, 2, label=total_princ)
ws.write(total_sheet_num+1+3+1, 3, label=total_interest)
...
for k in range(0, month_num):
    ...
    for ii in range(0, total_sheet_num):
        sheet = oriworkbook.sheet_by_name(list_of_all_sheetnames[ii])
        total_row_num = sheet.nrows
        dic = {}
        dic_princ = {}
        dic_interest = {}
...
#write a new excel sheet
ws.write(3, 5, label='每月现金流')
ws.write(4, 5, label='起始日')
ws.write(4, 6, label='租金总额')
ws.write(4, 7, label='本金总额')
ws.write(4, 8, label='收益总额')

for mm in range(0,month_num):
    ws.write(mm+1+3+1, 5, label=month_date_list[mm])

for nn in range(0,month_num):
    ws.write(nn+1+3+1, 6, label=total_per_month[nn])

for ff in range(0,month_num):
    ws.write(ff+1+3+1, 7, label=total_per_month_princ[ff])

for gg in range(0,month_num):
    ws.write(gg+1+3+1, 8, label=total_per_month_interest[gg])
...
ws.write(month_num+1+3+1, 5, label='总计')
ws.write(month_num+1+3+1, 6, label=total_of_all_month)
ws.write(month_num+1+3+1, 7, label=total_of_all_month_princ)
ws.write(month_num+1+3+1, 8, label=total_of_all_month_interest)
...
for kkk in range(0, season_num):
    ...
    for iii in range(0, total_sheet_num):
        sheet = oriworkbook.sheet_by_name(list_of_all_sheetnames[iii])
        total_row_num = sheet.nrows
        dic = {}
        dic_princ = {}
        dic_interest = {}
...
#write a new excel sheet
ws.write(3, 10, label='每季现金流')
ws.write(4, 10, label='起始日')
ws.write(4, 11, label='租金总额')
ws.write(4, 12, label='本金总额')
ws.write(4, 13, label='收益总额')

for mmm in range(0,season_num):
    ws.write(mmm+1+3+1, 10, label=season_date_list[mmm])

for nnn in range(0,season_num):
    ws.write(nnn+1+3+1, 11, label=total_per_season[nnn])

for fff in range(0,season_num):
    ws.write(fff+1+3+1, 12, label=total_per_season_princ[fff])

for ggg in range(0,season_num):
    ws.write(ggg+1+3+1, 13, label=total_per_season_interest[ggg])
...
ws.write(season_num+1+3+1, 10, label='总计')
ws.write(season_num+1+3+1, 11, label=total_of_all_season)
ws.write(season_num+1+3+1, 12, label=total_of_all_season_princ)
ws.write(season_num+1+3+1, 13, label=total_of_all_season_interest)
...
for kkkk in range(0, len(natural_date_list)):
    ...
    for iiii in range(0, total_sheet_num):
        sheet = oriworkbook.sheet_by_name(list_of_all_sheetnames[iiii])
        total_row_num = sheet.nrows
        dic = {}
        dic_princ = {}
        dic_interest = {}
...
#write a new excel sheet
ws.write(3, 15, label='自然季度现金流')
ws.write(4, 15, label='起始日')
ws.write(4, 16, label='租金总额')
ws.write(4, 17, label='本金总额')
ws.write(4, 18, label='收益总额')

for mmmm in range(0,len(natural_date_list)):
    ws.write(mmmm+1+3+1, 15, label=natural_date_list[mmmm])

for nnnn in range(0,len(natural_date_list)):
    ws.write(nnnn+1+3+1, 16, label=total_per_natural[nnnn])

for ffff in range(0,len(natural_date_list)):
    ws.write(ffff+1+3+1, 17, label=total_per_natural_princ[ffff])

for gggg in range(0,len(natural_date_list)):
    ws.write(gggg+1+3+1, 18, label=total_per_natural_interest[gggg])
...
ws.write(len(natural_date_list)+1+3+1, 15, label='总计')
ws.write(len(natural_date_list)+1+3+1, 16, label=total_of_all_natural)
ws.write(len(natural_date_list)+1+3+1, 17, label=total_of_all_natural_princ)
ws.write(len(natural_date_list)+1+3+1, 18, label=total_of_all_natural_interest)
